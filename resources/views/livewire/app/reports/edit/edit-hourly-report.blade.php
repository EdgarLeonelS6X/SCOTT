<div>
    <form wire:submit.prevent="saveReport" class="space-y-5">
        @foreach ($categories as $index => $category)
            ...
            @endphp
            <div wire:key="category-{{ $index }}" x-data="{
                open: true,
                editingName: false,
                ...
                <div class="p-6 border bg-white dark:bg-gray-800 rounded-2xl shadow-2xl mb-5">
                    ...
                    <div x-show="open" class="mt-6 space-y-6">
                        @foreach ($category['channels'] as $channelIndex => $channel)
                            <div wire:key="channel-{{ $index }}-{{ $channelIndex }}"
        ...
        : $channels->map(
            fn($c) => [
                ...
                'image' => $c->image,
                                        ...
                                        $selectedChannel = ($category['name'] === 'STINGRAY' ? $stingrayChannels : $channels)->firstWhere(
                                            'id',
                                            $channel['channel_id']))
