                        ...
                        <tbody>
                            @foreach ($report->channelTests->sortBy(fn($d) => $d->channel->number) as $test)
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    <td class="border px-3 py-2 text-left w-[600px]">
                                        <div class="flex flex-col gap-1">
                                            <div class="flex items-center gap-2 justify-between w-full">
                                                <div class="flex flex-col justify-start items-start flex-shrink-0">
                                                    <div>
                                                    <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-200 font-semibold text-xs mr-2">
                                                        {{ $test->channel->number }}
                                                    </span>
                                                ...
                                                <div class="flex flex-row gap-1 justify-end items-center flex-1">
                                                    @foreach ($profiles as $key => $value)
                                                    <div>
                                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold
                                                            {{ $key === 'high' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200' : '' }}
                                                            {{ $key === 'medium' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-200' : '' }}
                                                            {{ $key === 'low' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-200' : '' }}">
                                                            {{ ucfirst($key) }}: {{ $value }}
                                                        </span>
                                                    </div>
                                                    @endforeach
                                    ...
                                    <td class="border px-2 py-1">
                                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium
                                            {{ $test->high === 'CORRECTO' ? 'bg-green-200 text-green-800 dark:bg-green-900 dark:text-green-200' :
                                            ($test->high ? 'bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200' :
                                    ...
                                    <td class="border px-2 py-1">
                                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium
                                            {{ $test->medium === 'CORRECTO' ? 'bg-green-200 text-green-800 dark:bg-green-900 dark:text-green-200' :
                                            ($test->medium ? 'bg-yellow-200 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-200' :
                                            'bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-300') }}">
                                            {{ $test->medium ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="border px-2 py-1">
                                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium
                                            {{ $test->low === 'CORRECTO' ? 'bg-green-200 text-green-800 dark:bg-green-900 dark:text-green-200' :
                                            (str_contains($test->low, 'PAUSA') ? 'bg-orange-300 text-orange-800 dark:bg-orange-900 dark:text-orange-200' :
                                            ($test->low ? 'bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-200' :
                                            'bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-300')) }}">
                                            {{ $test->low ?? '-' }}
                                        </span>
                                    </td>
