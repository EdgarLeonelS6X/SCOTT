    ...
    <div class="container mx-auto py-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded px-3 py-1 shadow-sm">
                    <i class="fa-solid fa-calendar-days text-gray-400 mr-2"></i>
                    <label for="select-year" class="text-sm text-gray-700 dark:text-gray-200 mr-2">{{ __('Year') }}</label>
                ...
                @if(isset($devices) && $devices->count())
                    <div class="flex items-center bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded px-3 py-1 shadow-sm">
                        ...
                        <label for="select-device" class="text-sm text-gray-700 dark:text-gray-200 mr-2">{{ __('Device') }}</label>
            ...
            <div class="flex items-center gap-2">
                <a href="#" id="btn-export" class="inline-flex items-center gap-2 text-sm bg-white border border-gray-200 hover:bg-gray-50 px-3 py-2 rounded shadow-sm">
                    <i class="fa-solid fa-file-csv text-green-600"></i>
                    <span class="text-gray-700">{{ __('Export CSV') }}</span>
                </a>
                <a href="#" id="btn-new" class="inline-flex items-center gap-2 text-sm bg-primary-600 text-white hover:bg-primary-700 px-4 py-2 rounded shadow">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow p-4 h-[336px] relative overflow-hidden">
                <div class="flex items-center justify-between mb-3">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{ __('Shows monthly downloads for the selected device and year') }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <div id="chart-loading" class="text-sm text-gray-500 hidden">{{ __('Loading...') }}</div>
                        <button id="btn-refresh" class="text-sm text-gray-600 bg-gray-50 hover:bg-gray-100 px-2 py-1 rounded shadow-sm">
            ...
            <div class="flex flex-col space-y-4">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex-1 flex flex-col">
                    ...
                    <div class="mt-3 text-xs text-gray-500 text-center">{{ __('Distribution across device categories or regions') }}</div>
                </div>
        ...
        <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-2">
                    <input id="history-search" type="search" placeholder="{{ __('Search...') }}" class="border rounded px-2 py-1 text-sm" />
                    <a href="#" id="history-export" class="text-sm text-gray-600 hover:underline">{{ __('Export CSV') }}</a>
                </div>
            </div>

            <div class="overflow-x-auto">
                <table class="min-w-full text-sm text-left text-gray-600 dark:text-gray-300 divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tbody id="history-body" class="bg-white dark:bg-gray-800">
                        <tr class="border-t hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-4 py-3 text-gray-500" colspan="5">{{ __('No data. Use the controls above to load or create monthly download reports.') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
