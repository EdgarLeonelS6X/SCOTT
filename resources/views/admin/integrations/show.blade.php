<x-admin-layout :breadcrumbs="[
    ...
    <x-slot name="action">
        <div class="hidden lg:flex space-x-2">
            ...
            <a href="{{ route('admin.integrations.edit', $url) }}"
    ...
    <div
        ...
        <div class="p-8 space-y-6">
            <div class="flex justify-between items-center gap-3">
                ...
                @php
                    $isGrafana = strtolower($url->type) === 'grafana';
                @endphp
                <span class="inline-flex items-center px-2.5 py-2 text-sm font-bold
                        {{ $isGrafana ? 'text-orange-900 bg-orange-200 border-orange-300 dark:bg-orange-900 dark:text-orange-100 dark:border-orange-800' : 'text-blue-900 bg-blue-200 border-blue-300 dark:bg-blue-900 dark:text-blue-100 dark:border-blue-800' }}
                        rounded-full border shadow-sm">
                    <i class="fa-solid {{ $isGrafana ? 'fa-chart-pie' : 'fa-plug' }} mr-1"></i>
                    {{ __($url->type) }}
                </span>
            </div>
            <div class="flex flex-col gap-6 mt-4">
                <div class="flex flex-col gap-1">
                    ...
                    <x-input type="text" :value="$url->url" disabled class="w-full" />
                </div>
                <div class="flex flex-col gap-1">
                    <x-label><i class="fa-solid fa-key mr-1"></i> {{ __('API Key') }}</x-label>
                    <div class="flex items-center gap-2">
                        @if($url->key)
                            <x-input type="text" :value="str_repeat('*', strlen($url->key->key))" disabled
    ...
    <form action="{{ route('admin.integrations.destroy', $url) }}" method="POST" id="delete-form">
        @csrf
        @method('DELETE')
    </form>
    @push('js')
        <script>
            ...
            function toggleKeyVisibility() {
                ...
                const realKey = @json($url->key ? $url->key->key : '');
